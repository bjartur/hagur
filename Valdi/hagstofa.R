...
#
dalknofn <- c("Ar", "Eigniralls", "MedaltalEigna", "MidgildiEigna", "Fasteignir", "Okutaeki", "Innlan", "Verdbrjef", "AdrarEignir", "Skuldiralls", "MedaltalSkulda", "MidgildiSkulda", "Ibudarlan", "Adrarskuldir", "EigidFje", "EgidFjeiFasteign", "Eigidfje annad", "Fjoldiihop", "MedaltalSkuldaSkuldsettra", "FjoldiSkuldsettra")
...
#source('núşákrónur.R') # Leiğrétta fyrir verğlag

aldursflokkar <- list(u24,aldur2529,aldur3034,aldur3539,aldur4044,aldur4549,aldur5054,aldur5559,aldur6066,y67)

# Eigiğ fé eftir aldri, á verğlagi hvers árs
eigidfjeAldurs <- sapply(aldursflokkar, function(aldur) {
  t(
    leidretta(aldur$Eigidfje/aldur$Fjoldiihop) #listi eigiğ fé tiltekna aldurhópsins hvert ár tímabilsins
  )
})
