...
# Notkun: eignirvstekjur = function(hopar, breytur, aldurstrue)
# Fyrir:
...
#  breytur er strengjavigur sem inniheldur nöfnin úr hagstofa.R á breytunum sem setja skal á myndina 
    # ferlanir sem samsvara þeim verða plottaðir
    # t.d. c("EigidFje", "EigidFjeiFasteign", "EigidFjeAnnad)
    ...
    # "Fjoldiihop")
    # (sleppum meðaltölunum og þeim dalkum sem ekki eru mældir i milljón króna)
...
eignirvstekjur = function(hoparvaldir,breyturvaldar, aldurstrue)
{
    ...
    linutypur = c(1,2,3) # fyrir lty    solid/dashed/dotted
# hoparvaldir = c(1:4)
...
# set upp ramma til að teikna á. þarf fyrst að finna hæsta og lægsta gildið til að stilla rammann
        plot(arin, rep(0,17), xlab = "Ár", type = "l", lty = 1, ylab = "Fjárhagsstaða að meðaltali [m. króna]", xaxp = c(arin[1],arin[17], 8), ylim = c(ymin,ymax) ) 
...
# MAX 3 mismunandi breytur i einu. type = "l", "b", "p"
    ...
    else
    {
        ...
        title("Landshlutar - höfuðborgarsvæði vs. landsbyggð")    
        for(i in breytunumer)
        {  
            ...
            for(j in hoparvaldir)
            {
                lines(arin, stadladgildi[,j], type = "b", col = litir[j])
            }
...
breyturvaldar = c("EigidFje", "Innlan")

eignirvstekjur(4:6,breyturvaldar,TRUE)
